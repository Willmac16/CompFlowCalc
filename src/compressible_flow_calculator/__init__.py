...
class NormalShockRatio:
    def __init__(self, gamma, mach):
        ...
        self.stag_ratio = (
            self.temp_ratio ** (-gamma / (gamma - 1)) * self.pressure_ratio
        )
...
class ExpansionWaveRatio:
    def __init__(self, gamma, mach):
        ...
        self.pressure_ratio = 1 / (1 + (gamma - 1) / 2 * mach) ** (
            2 * gamma / (gamma - 1)
        )
...
def tbm_implicit(gamma, theta, beta, mach):
    ...
    rhs = (
        2
        / math.tan(beta)
        * (mach**2 * math.sin(beta) ** 2 - 1)
        / (mach**2 * (gamma + math.cos(2 * beta)) + 2)
    )
    return rhs - lhs
    ...
    def __init__(self, gamma, mach=math.nan, beta=math.nan, theta=math.nan):
        if not math.isnan(beta) and not math.isnan(mach):
            ...
            self.mach = root_scalar(
                lambda mach: tbm_implicit(gamma, theta, beta, mach), bracket=[0.1, 1000]
            ).root
            ...
            self.beta = root_scalar(
                lambda beta: tbm_implicit(gamma, theta, beta, mach),
                x0=0.25,
...
class ObliqueShock:
    def __init__(self, gamma, mach=math.nan, beta=math.nan, theta=math.nan):
        ...
        self.theta = math.atan(
            2
            / math.tan(beta)
            * (mach_one**2 * math.sin(beta) ** 2 - 1)
            / (mach_one**2 * (gamma + math.cos(2 * beta)) + 2)
        )
...
def prandtlMeyerNu(gamma, mach):
    return math.sqrt((gamma + 1) / (gamma - 1)) * math.atan(
        math.sqrt((gamma - 1) / (gamma + 1) * (mach**2 - 1))
...
class PrandtlMeyer:
    def __init__(self, gamma, mach_one=math.nan, mach_two=math.nan, theta=math.nan):
        if not math.isnan(mach_one) and not math.isnan(mach_two):
            ...
            self.theta = prandtlMeyerNu(gamma, mach_two) - prandtlMeyerNu(
                gamma, mach_one
            )
        elif not math.isnan(mach_one) and not math.isnan(theta):
            self.mach_one = mach_one
            self.theta = theta
            self.mach_two = root_scalar(
                lambda mach_two: prandtlMeyerNu(gamma, mach_two)
                - prandtlMeyerNu(gamma, mach_one)
                - theta,
                bracket=[mach_one, 1000],
            ).root
        elif not math.isnan(mach_two) and not math.isnan(theta):
            self.mach_two = mach_two
            self.theta = theta
            self.mach_one = root_scalar(
                lambda mach_one: prandtlMeyerNu(gamma, mach_two)
                - prandtlMeyerNu(gamma, mach_one)
                - theta,
                bracket=[1e-6, mach_two],
            ).root
...
if __name__ == "__main__":
    ...
    print(
        "Theta @ M2, Beta = 50˚:",
        math.degrees(ThetaBetaMach(GAMMA, mach=2, beta=math.radians(50)).theta),
    )

    assert (
        abs(
            pressure_inverse_isentropic(GAMMA, isentropic(GAMMA, 2).pressure_ratio).mach
            - 2.0
        )
        < 1e-6
    )
    ...
    print(
        f"Pressure ratio of a Mach 2 Normal Shock {normal_shock(GAMMA, 2).pressure_ratio:.5f}"
    )
